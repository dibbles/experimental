...
func (r Resource) deleteWebhook(request *restful.Request, response *restful.Response) {
	...
	for _, hook := range webhooks {
		if hook.Name == name && hook.Namespace == namespace {
			found = true
			if len(webhooks) == 1 {
				logging.Log.Debugf("No other pipelines triggered by this GitHub webhook, deleting webhook")
				remaining = 0
				sanitisedURL := gitServer + "/" + gitOwner + "/" + gitRepo
...
// Retrieve webhook entry fro configmap for the GitHub URL
